...
{
	return (List *) pList->vGC.pContainer ;
}
...
{
	return pList->vGC.nReferenceCount + 1 ;
}
...
{
	pList->vGC.lErrorOnAssignment = 1 ;
}
...
{
	return pList->vGC.lErrorOnAssignment ;
}
...
{
	return pList->vGC.lErrorOnAssignment2 ;
}

RING_API void ring_list_enableerroronassignment2 ( List *pList )
{
	pList->vGC.lErrorOnAssignment2 = 1 ;
}
...
void * ring_poolmanager_allocate ( RingState *pRingState,size_t nSize )
{
	...
	if ( (pRingState->vPoolManager.pCurrentItem == NULL) && (pRingState->vPoolManager.pBlockStart == NULL)  && (pRingState->lStartPoolManager) && (pRingState->lOnlyTokens == 0) ) {
	...
	else if ( ( (nSize > sizeof(PoolData)) &&  (nSize <= sizeof(PoolDataL2)) ) && (pRingState->vPoolManager.pCurrentItemL2 != NULL) ) {
	...
	else if ( ( (nSize > sizeof(PoolDataL2)) &&  (nSize <= sizeof(PoolDataL3)) ) && (pRingState->vPoolManager.pCurrentItemL3 != NULL) ) {
...
void ring_poolmanager_deleteblockfromsubthread ( RingState *pSubRingState,RingState *pMainRingState )
{
	...
	while ( pMemory != NULL ) {
		...
		**  Then it will not be deleted and will not return to main thread (To avoid lock/unlock & slow performance) 
