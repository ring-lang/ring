            ...
            if ( (strcmp(cStr,"main") != 0 ) && (pVM->nCallMethod != 1) && (y != 2) ) {
                ...
                if ( pVM->nInsideBraceFlag == 0 ) {
                    if ( nPerformance == 1 ) {
                        ...
                        RING_VM_IR_ITEMSETPOINTER(RING_VM_IR_ITEM(3),ring_list_getstring(pList2,RING_FUNCMAP_FILENAME));
                        RING_VM_IR_ITEMSETINT(RING_VM_IR_ITEM(4),ring_list_getint(pList3,RING_FUNCCL_METHODORFUNC));
                        RING_VM_IR_ITEMSETINT(RING_VM_IR_ITEMATINS(RING_VM_PC_PREVINS,1),ring_list_getint(pList3,RING_FUNCCL_LINENUMBER));
                    }
                }
                else {
                    /*
