...
void ring_vm_explog ( VM *pVM )
{
    ...
    if ( RING_VM_STACK_ISNUMBER ) {
        ...
        RING_VM_STACK_POP ;
        if ( RING_VM_STACK_ISNUMBER ) {
            RING_VM_STACK_SETNVALUE( exp( nNum1 * log( (RING_VM_STACK_READN) )));
        }
        else if ( RING_VM_STACK_ISSTRING ) {
            nNum2 = ring_vm_stringtonum(pVM,RING_VM_STACK_READC);
            RING_VM_STACK_SETNVALUE( exp( nNum1 * log( (RING_VM_STACK_READN) )));
...
/*
**  compare 
**  Here we dont't use conversion string/number before comparing 
**  Because we use strict operation where types must be equal 
*/

void ring_vm_equals ( VM *pVM )
{
    String *cStr1,*cStr2  ;
    double nNum1,nNum2  ;
    RING_LIST_CHECKOPERATIONONSUBLIST ;
