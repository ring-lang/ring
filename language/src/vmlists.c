        ...
        if ( RING_VM_STACK_OBJTYPE == RING_OBJTYPE_VARIABLE ) {
            ...
            if ( pList2->nReferenceCount ) {
                ...
                if ( pList2->lNewRef ) {
                    ...
                    ring_list_updatenestedreferences_gc(pVM->pRingState,pList2,NULL,RING_LISTREF_INC);
                }
            }
            else {
                ring_vm_list_copy(pVM,pList4,pList2);
    ...
    if ( (RING_VM_STACK_ISSTRING) && (pVM->nBeforeEqual <= 1) ) {
        ...
        if ( pVar->nReferenceCount ) {
            ...
            if ( pVar->lNewRef ) {
                ...
                ring_list_updatenestedreferences_gc(pVM->pRingState,pVar,NULL,RING_LISTREF_INC);
            }
        }
        else {
            if ( pVar->lCopyByRef ) {
