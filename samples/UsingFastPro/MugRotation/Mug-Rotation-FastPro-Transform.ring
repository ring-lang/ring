...
### 2025 -- Updated: replaced mylibCalcMug.dll with FastPro transform3d()

Load "fastpro.ring"       ### <<< FastPro C-extension: provides transform3d()
Load "guilib.ring"
Load "stdlibcore.ring"

? "FastPro transform3d() loaded"
...
PI = 3.14159
NbrRotation = 8.2 * PI
IncRotation = 0.1         ### 0.1 Rad -- Increment rotation by: in seconds
...
Func DrawMatrix()
   ...
   // Two Complete Rotations = 88PI
   
   Count = 0
   R = 0
   StartTime = Clock()   // 8*Pi rotations
   
   while R <= NbrRotation     // 8Pi = 2 complete rotations
      t1 = Clock()
        ...
        Rz = R * eRotZV.text()               
         ...
         Fx = eReflXV.text()  
         Fy = eReflYV.text()  
         Fz = eReflZV.text()          // Reflect -1,  NoReflect = 1
         ...
         F  = [[  Fx,   0,   0,  0],  // Reflection  X-Y-Z plane
            ...
            C = MatrixMultiply(Y,X)   // Rotation Y:   
            C = MatrixMultiply(Z,C)   // Rotation Z:
            C = MatrixMultiply(S,C)   // Scale      -- make bigger/smaller size drawing
            C = MatrixMultiply(T,C)   // Translate  -- move drawing to different position
            C = MatrixMultiply(H,C)   // Shear      -- distort drawing in X or Y or Z direction
           FC = MatrixMultiply(F,C)   // Reflection -- mirror image of drawing 
            ...
            // Apply the Transformation Matrix to Each New Point in Mug Array
                ...
                ###----------------------------------------------------------
                ### FastPro transform3d() -- replaces mylibCalcMug.dll
                ### ring_transform3d in fastpro.c
                ### Applies FC (4x4) to every row of RC (Nx4) in-place
                    
            RC    = ShapeMug            // Keep ShapeMug array as-is

            tCal1 = Clock()

            transform3d(FC, RC)         // <<<=== FastPro: FC x RC => RC  (in-place)

            tCal2 = Clock()
            tCal3 = tCal2 - t1          // Ring Transforms(6) + transform3d time
      ...
      t3 = t2 - t1
      eTimeXV.setText(""+ tCal3)      // t3 TimeCycle: MSec:
     ...
     EndTime = Clock()   // 8*Pi rotations
    DiffTime = EndTime - StartTime
   
    See "Total 8*PI Rotations: "+ DiffTime +" msecs" +" IncRotation : "+ IncRotation  +" CountCycles: "+ Count +nl
...
Func DoDraw(C)      // Called by DoDraw(RC) 
   ...
   if ShapeType = "ShapeMug"
      ...
      for i = 1 to len(ShapeMug) -1
            daVinci.drawPoint( C[i][1], C[i][2])  // Draw X-Y Point
      next
   ok
...
FastPro transform3d() replaces mylibCalcMug.dll
Load "fastpro.ring" at top -- no DLL loading required

Original with mylibCalcMug.dll:
Total 8*PI Rotations: 19994 msecs IncRotation : 0.10 CountCycles: 258
