...
func mysql_file( path )
   if IsNULL( db_con )
      see "You are not conteccted. Use mysql_connect_now()" + NL
      bye
   ok
   file = read( path )
   return mysql_escape_string( db_con , file )
...
func mysql_fill_combobox( combobox , table_name , column_name )
   ...
   combobox.clear()
   for row=2 to len(r)
      combobox.AddItem( r[row][1] , 0 )
   next

func mysql_fill_list( listwidget , table_name , column_name )
   if lower( type(listwidget) ) != "object"
      see "Error: Use object only."
      bye
   ok
   if lower( classname(listwidget) ) != "listwidget"
      see "Error: Use listwidget only."
      bye
   ok
   stmt = "select " + column_name + " from " + table_name
   r = mysql_get( stmt )
